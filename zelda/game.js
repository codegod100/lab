...
const MINIMAP_SIZE = 100; // Size of the minimap square
const MINIMAP_SCALE = 1; // Scale factor for the minimap
const MINIMAP_ENTITY_SIZE = 3; // Larger dots for entities
...
function drawMinimap() {
  ...
  const playerWorldX = worldX + playerX / tileSize;
  const playerWorldY = worldY + playerY / tileSize;

  // Calculate minimap view range centered on player
  const tilesInView = MINIMAP_SIZE / MINIMAP_SCALE;
  const startX = Math.floor(playerWorldX - tilesInView / 2);
  const startY = Math.floor(playerWorldY - tilesInView / 2);

  // Draw world tiles on minimap
  for (let y = 0; y < tilesInView; y++) {
    for (let x = 0; x < tilesInView; x++) {
      const worldTileX = Math.floor(startX + x);
      const worldTileY = Math.floor(startY + y);
      ...
      if (chunk) {
        ...
        const minimapX = canvas.width - MINIMAP_SIZE - 10 + x * MINIMAP_SCALE;
        const minimapY = 10 + y * MINIMAP_SCALE;
        ...
        ctx.fillRect(minimapX, minimapY, MINIMAP_SCALE, MINIMAP_SCALE);
      }
    }
  }
  ...
  for (const [_, entity] of entities) {
    const entityTileX = entity.x / tileSize;
    const entityTileY = entity.y / tileSize;

    const relativeX = entityTileX - startX;
    const relativeY = entityTileY - startY;

    if (
      relativeX >= 0 &&
      relativeX <= tilesInView &&
      relativeY >= 0 &&
      relativeY <= tilesInView
    ) {
      const minimapX =
        canvas.width - MINIMAP_SIZE - 10 + relativeX * MINIMAP_SCALE;
      const minimapY = 10 + relativeY * MINIMAP_SCALE;

      ctx.fillStyle = "black";
      ctx.fillRect(
        minimapX - 1,
        minimapY - 1,
        MINIMAP_ENTITY_SIZE + 2,
        MINIMAP_ENTITY_SIZE + 2,
      );

      ctx.fillStyle = entity.hostile ? "#FF0000" : "#00BFFF";
      ctx.fillRect(
        minimapX,
        minimapY,
        MINIMAP_ENTITY_SIZE,
  ...
  const playerMinimapX =
    canvas.width - MINIMAP_SIZE - 10 + (tilesInView / 2) * MINIMAP_SCALE;
  const playerMinimapY = 10 + (tilesInView / 2) * MINIMAP_SCALE;
  ...
  ctx.font = "12px Arial";
  ctx.fillStyle = "white";
  ctx.fillText("Minimap", canvas.width - MINIMAP_SIZE - 10, 8);
}
// UI text
function drawUI() {
  ctx.font = "12px Arial";
    ...
    if (!entity.hostile) {
      ...
      for (const [id, group] of friendlyGroups) {
        for (const friendly of group) {
          ...
          if (dist < 5 * tileSize) {
            // Consider NPCs within 5 tiles as a group
            group.push(entity);
            foundGroup = true;
            break;
          }
    ...
    .map((_, y) =>
      Array(chunkSize)
        ...
        .map((_, x) => {
          ...
          if (
            (Math.random() < 0.03 ||
              (noiseValue > 0.7 && Math.random() < 0.08)) &&
            !isMainRoad(worldX, worldY) &&
            !isNearRoad(worldX, worldY)
          ) {
            spawnEntity(worldX, worldY, "enemy", true);
          }
          // Increased friendly NPC spawn rate in town centers
          if (
            isTownCenter(worldX, worldY) &&
            Math.random() < 0.15 &&
            isNearRoad(worldX, worldY)
          ) {
          ...
          if (isTownCenter(worldX, worldY)) {
            ...
            if (localX > 14 && localX < 18 && localY > 14 && localY < 18)
              return TOWN_HALL;
            if (localX > 12 && localX < 20 && localY > 12 && localY < 20)
              return MARKET;
            if (
              (localX === 12 || localX === 20) &&
              localY >= 12 &&
          ...
          if (!isNearRoad(worldX, worldY)) {
            if (
              noiseValue > 0.7 &&
              Math.hypot((worldX % 64) - 32, (worldY % 64) - 32) < 10
            )
              return LAKE;
...
function gameLoop() {
  ...
  const visibleChunksX = Math.ceil(viewportTilesX / chunkSize) + 2; // Add buffer
  const visibleChunksY = Math.ceil(viewportTilesY / chunkSize) + 2;
  const currentChunkX = Math.floor(worldX / chunkSize);
  const currentChunkY = Math.floor(worldY / chunkSize);

  // Pre-generate chunks in visible range
  for (let y = -visibleChunksY; y <= visibleChunksY; y++) {
    for (let x = -visibleChunksX; x <= visibleChunksX; x++) {
      const chunkX = currentChunkX + x;
      const chunkY = currentChunkY + y;
      const chunkKey = `${chunkX},${chunkY}`;

      if (!worldChunks.has(chunkKey)) {
        worldChunks.set(chunkKey, generateChunk(chunkX, chunkY));
      }
    }
  }
  ...
  for (let y = 0; y < viewportTilesY; y++) {
    for (let x = 0; x < viewportTilesX; x++) {
      ...
      if (chunk) {
        ...
        // Default to grass if tile is undefined
        const tileType = tile !== undefined ? tile : GRASS;
  ...
  for (const [key, entity] of entities) {
    ...
    if (
      ...
      entity.update(
        playerX + worldX * tileSize,
        playerY + worldY * tileSize,
        entities,
      );
  ...
  switch (e.key) {
    ...
    case "d":
      ...
      if (friendlyGroup && friendlyGroup.length > 0) {
        // Calculate average position of the group
        const avgX =
          friendlyGroup.reduce((sum, e) => sum + e.x, 0) / friendlyGroup.length;
        const avgY =
          friendlyGroup.reduce((sum, e) => sum + e.y, 0) / friendlyGroup.length;
